...
export const Footer = () => (
  <Box as="footer" bg="brand.100" color="white" py="64px">
    <Box maxW="7xl" px="8" mx="auto">
      ...
      >
        ...
        >
          {links.map((group, idx) => (
            <Box key={idx}>
              <Text fontWeight="bold" mb="4" color='brand.800'>
                {group.title}
              </Text>
      ...
      >
        <Wrap id="bottom" spacing={{ base: '4', lg: '8' }} mt={{ base: '4', lg: '0' }}>
          <WrapItem>
            <Box color='brand.900'>&copy; Glowlabs</Box>
          </WrapItem>
          {footerLinks.map((link, idx) => (
            <WrapItem key={idx}>
              <Box as="a" href={link.href} color='brand.900'>
                {link.label}
              </Box>
            </WrapItem>
          ))}
